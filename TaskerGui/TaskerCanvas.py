...
class TaskerCanvas(ttk.Frame):
    ...
    def __init__(self, mainframe, width=500, height=500):
        ...
        # self.canvas.get_tk_widget().bind('<ButtonPress-1>', self.move_from)
        # self.canvas.get_tk_widget().bind('<B1-Motion>',     self.move_to)
        # self.canvas.get_tk_widget().bind('<MouseWheel>', self.wheel)  # with Windows and MacOS, but not Linux
        # self.canvas.get_tk_widget().bind('<Button-5>',   self.wheel)  # only with Linux, wheel scroll down
        # self.canvas.get_tk_widget().bind('<Button-4>',   self.wheel)  # only with Linux, wheel scroll up

        # self.canvas.get_tk_widget().bind('<Double-Button-1>', self.zoomin)
        # self.canvas.get_tk_widget().bind('<Double-Button-3>', self.zoomout)
    ...
    def event_receive(self,event):     #Event can be an array, where the event name is first
        if len(event) > 0:             #Details for the event "like filename come later
            ...
            elif type == "TaskerTreeView::raster_menu_select":
                if len(event) == 5:
                    choice = event[1]
                    rastername = event[2]
                    imagename= event[3]
            ...
            elif ( type == "TaskerButtonBar::zoomIn" or
                   type == "TaskerMenuBar::viewZoomIn" ):
                xy.x, xy.y = pyautogui.position()
                self.zoomin(xy)
            elif( type == "TaskerButtonBar::plot"):
                if plotButton == False:
                    self.plotPoint()
                else:
                    self.canvas.get_tk_widget().bind('<ButtonPress-1>', self.move_from)
                    self.canvas.get_tk_widget().bind('<B1-Motion>',     self.move_to)
                    self.canvas.get_tk_widget().bind('<Double-Button-1>', self.zoomin)
            elif ( type == "TaskerButtonBar::zoomOut" or
                   type == "TaskerMenuBar::viewZoomOut" ):
                xy.x, xy.y = pyautogui.position()
                self.zoomout(xy)
            elif( type == "TaskerButtonBar::grayScale"):
                self.grayScale()
            elif(type == "TaskerButtonBar::histogram"):
                self.greyscaleHistogram()
            elif(type == "TaskerApp::paned_window_resized"):
    ...
    def scroll_y(self, *args, **kwargs):
        ...
        self.canvas.get_tk_widget().yview(*args, **kwargs)  # scroll vertically
        self.show_image()  # redraw the image

    def scroll_x(self, *args, **kwargs):
        ''' Scroll canvas horizontally and redraw the image '''
        self.canvas.get_tk_widget().xview(*args, **kwargs)  # scroll horizontally
        self.show_image()  # redraw the image

    def move_from(self, event):
        ''' Remember previous coordinates for scrolling with the mouse '''
        self.canvas.get_tk_widget().scan_mark(event.x, event.y)

    def move_to(self, event):
        ''' Drag (move) canvas to the new position '''
        self.canvas.get_tk_widget().scan_dragto(event.x, event.y, gain=1)
        self.show_image()  # redraw the image

    def zoomout(self,event):
        ''' Zoom-in with double-click on mouse-btn-1 '''
        x = self.canvas.get_tk_widget().canvasx(event.x)
        y = self.canvas.get_tk_widget().canvasy(event.y)
        bbox = self.canvas.get_tk_widget().bbox(self.container)  # get image area
        ...
        self.canvas.get_tk_widget().scale('all', x, y, scale, scale)  # rescale all canvas objects
        self.show_image()
    ...
    def zoomin(self,event):
        ...
        i = min(self.canvas.get_tk_widget().winfo_width(), self.canvas.get_tk_widget().winfo_height())
    ...
    def fileSelect(self, filename):
        ...
        self.container = self.canvas.get_tk_widget().create_rectangle(0, 0, 500, 500, width=0)
        self.show_image()

    def rasterSelect(self,filename,imagename,rastername):
        ## for now, just show the selected raster:
        ## later need to be fancier.....
        #messagebox.showinfo("Information","TaskerCanvas::rasterSelect: inputs= "+filename+", "+imagename+", "+rastername)
        f = h5py.File(filename, 'r')
        rasterpath = "/"+imagename+"/"+rastername
        dset=f[rasterpath]
        data = np.array(dset[:,:])
        #messagebox.showinfo("Information","TaskerCanvas::rasterSelect: data.shape= "+str(data.shape[0])+", "+str(data.shape[1]) )
    ...
    def rasterChoiceSelect(self,choice,filename,imagename,rastername):
        ## for now, just show the selected raster:
        ## later need to be fancier.....
        #messagebox.showinfo("Information","TaskerCanvas::rasterChoiceSelect: inputs= "+choice+", "+filename+", "+imagename+", "+rastername)
        if choice == "gray":
    ...
    def rasterGraySelect(self,filename,imagename,rastername):
        ...
        data = np.array(dset[:,:])
        #messagebox.showinfo("Information","TaskerCanvas::rasterSelect: data.shape= "+str(data.shape[0])+", "+str(data.shape[1]) )
        #print("TaskerCanvas::rasterSelect: data.shape= "+str(data.shape[0])+", "+str(data.shape[1]) )
        self.image = Image.fromarray(data)
        self.image = self.image.convert('L') # converts to grayscale
        self.image = self.image.crop((0,0,self.canvas_width,self.canvas_height))
        self.container = self.canvas.get_tk_widget().create_rectangle(0,0,self.canvas_width,self.canvas_height)
        #self.greyscaleHistogram(rastername)
        global raster
        raster = rastername
        #print(rastername, raster)
    ...
    def plotPoint(self):
        ...
        plotButton == True
        self.canvas.get_tk_widget().bind('<ButtonPress-1>', self.onStart)   
        self.canvas.get_tk_widget().bind('<B1-Motion>',     self.onGrow)     
        self.canvas.get_tk_widget().bind('<Double-1>',      self.onClear)     
        self.canvas.get_tk_widget().bind('<ButtonPress-3>', self.onMove)     
        self.drawn  = None
        self.kinds = [self.canvas.get_tk_widget().create_oval, self.canvas.get_tk_widget().create_rectangle]
    ...
    def show_image(self, event=None):
        ...
        if not hasattr(self,'container'):
            self.container = self.canvas.get_tk_widget().create_rectangle(0, 0, self.canvas_width,self.canvas_height, width=0)

        bbox1 = self.canvas.get_tk_widget().bbox(self.container)  # get image area
        # Remove 1 pixel shift at the sides of the bbox1
        bbox1 = (bbox1[0] + 1, bbox1[1] + 1, bbox1[2] - 1, bbox1[3] - 1)
        bbox2 = (self.canvas.get_tk_widget().canvasx(0),  # get visible area of the canvas
                 self.canvas.get_tk_widget().canvasy(0),
                 self.canvas.get_tk_widget().canvasx(self.canvas.get_tk_widget().winfo_width()),
                 self.canvas.get_tk_widget().canvasy(self.canvas.get_tk_widget().winfo_height()))
        ...
        if bbox[1] == bbox2[1] and bbox[3] == bbox2[3]:  # whole image in the visible area
            bbox[1] = bbox1[1]
            bbox[3] = bbox1[3]
        self.canvas.get_tk_widget().configure(scrollregion=bbox)  # set scroll region
        ...
        if int(x2 - x1) > 0 and int(y2 - y1) > 0:  # show image if it in the visible area
            ...
            imageid = self.canvas.get_tk_widget().create_image(max(bbox2[0], bbox1[0]), max(bbox2[1], bbox1[1]),
                                               anchor='nw', image=imagetk)
            self.canvas.get_tk_widget().lower(imageid)  # set image into background
            self.canvas.get_tk_widget().imagetk = imagetk  # keep an extra reference to prevent garbage-collection
