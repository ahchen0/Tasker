...
class PythonConsole(tk.Frame):
    def __init__(self,parent=None):
        tk.Frame.__init__(self, parent)
        self.parent = parent
        self.createWidgets()

        # get the path to the runpython.py file assuming it is in the same folder
        consolePath = os.path.join(os.path.dirname(__file__),"runpython.py")
    ...
    def createWidgets(self):
        ...
        self.ttyText = tk.Text(self.frame, wrap=tk.WORD, yscrollcommand=self.vbar.set)
        self.ttyText.pack(side="left",fill=tk.BOTH,expand=True)
        self.vbar['command'] = self.ttyText.yview

if __name__ == '__main__':
    root = tk.Tk()
    root.config(background="red")
    main_window = PythonConsole(root)
